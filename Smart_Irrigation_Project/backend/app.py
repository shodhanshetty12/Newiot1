...
def run_simulation():
    ...
    while simulation_running and simulation_index < len(simulation_data):
        ...
        if row["pump_status"] == 1:
            ...
            last_pump_status = "ON"
            try:
                set_setting("last_pump_status", last_pump_status)
            except Exception:
                pass
            try:
...
# NOTE: Hardware pump control is handled by the `hardware` blueprint at
# `/api/hardware/pump`. The duplicate app-level route was removed to avoid
# conflicts and ensure a single source of truth for this endpoint.
...
def api_mode():
    ...
    if request.method == "GET":
        try:
            persisted = get_setting("system_mode", current_mode) or current_mode
            return jsonify({"mode": persisted})
        except Exception:
    ...
    current_mode = mode
    try:
        set_setting("system_mode", current_mode)
    except Exception:
        pass
...
def api_status():
    try:
        mode_val = get_setting("system_mode", current_mode) or current_mode
    except Exception:
        mode_val = current_mode
    try:
        pump_val = get_setting("last_pump_status", last_pump_status) or last_pump_status
    except Exception:
        pump_val = last_pump_status
    return jsonify({
        "mode": mode_val,
...
def metrics_water():
    ...
    rows = c.fetchall()
    conn.close()
    resp = jsonify([{"bucket": r[0], "liters": r[1] or 0} for r in rows])
    resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    resp.headers["Pragma"] = "no-cache"
    return resp
