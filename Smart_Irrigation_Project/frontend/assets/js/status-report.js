(function (window, document) {
  ...
  function initCharts() {
    ...
    if (!window.Chart || !waterCumCtx || !moistureCtx || !tempCtx || !humCtx) {
      window.setTimeout(initCharts, 150);
      return;
    }

    // Create charts only once (removed waterFlow chart)
    charts.waterCumulative = createLineChart(
      waterCumCtx,
      "Cumulative Water (24h)",
      "#00e5ff",
      "rgba(0,229,255,0.18)"
    );
  ...
  function processWaterData(canonical) {
    ...
    if (window.GlobalState) {
      ...
      if (state.lastTimestamp && tsMs <= state.lastTimestamp) {
        // Out of order timestamp - ignore
        return null;
      }
      window.GlobalState.updateState({ lastTimestamp: tsMs });
    }
    ...
    // Get cumulative from global state first
    let cumulative = 0;
    if (window.GlobalState) {
      const state = window.GlobalState.getState();
      cumulative = state.lastCumulativeWater || 0;
    } else {
      cumulative = waterState.cumulative;
    }
  ...
  async function fetchData() {
    ...
    if (window.GlobalState) {
      const state = window.GlobalState.getState();
      // If in SIMULATION mode and simulation is not running, don't fetch
  ...
  function updateCharts(canonical) {
    ...
    // Removed waterFlow chart - only showing cumulative water
  ...
  function updateUI(canonical) {
    ...
    if (waterEl && typeof cumulative === "number") {
      waterEl.textContent = `${cumulative.toFixed(1)} L`;
    }
    if (updatedAt) {
  ...
  async function tick() {
    ...
    // Check if we should poll (use global state as source of truth)
    if (window.GlobalState) {
      const state = window.GlobalState.getState();
      // If in SIMULATION mode and simulation is not running, don't poll
      if (state.APP_MODE === "SIMULATION" && !state.simulationRunning) {
        return;
      }
    }
  ...
  function bootstrap() {
    ...
    // Load global state and restore settings
    if (window.GlobalState) {
