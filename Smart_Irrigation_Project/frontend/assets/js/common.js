(function (window, document) {
      ...
      if (res.ok) {
        ...
        if (window.GlobalState) {
          window.GlobalState.updateState({ 
            APP_MODE: normalized === "hardware" ? "HARDWARE" : "SIMULATION" 
          });
        }
        
        // Update APP_MODE if available
        if (window.APP_MODE) {
          window.APP_MODE.setMode(normalized === "hardware" ? "HARDWARE" : "SIMULATION");
        }
      }
    } catch (error) {
  ...
  async function syncModeFromBackend() {
    ...
    if (window.GlobalState) {
      ...
      const globalState = window.GlobalState.getState();
      if (globalState.APP_MODE) {
        state.mode = globalState.APP_MODE === "HARDWARE" ? "hardware" : "simulation";
        updateModeToggles();
        return;
      }
    }
    ...
    try {
      ...
      if (window.GlobalState) {
        window.GlobalState.updateState({ 
          APP_MODE: state.mode === "hardware" ? "HARDWARE" : "SIMULATION" 
        });
      }
  ...
  function updateNavBadges(status) {
    // Get values from global state first (source of truth)
    ...
    if (window.GlobalState) {
      window.GlobalState.loadState();
      const globalState = window.GlobalState.getState();
      modeLabel = globalState.APP_MODE === "HARDWARE" ? "hardware" : "simulation";
      pumpLabel = globalState.pumpState || "OFF";
      waterValue = typeof globalState.lastCumulativeWater === "number"
        ? globalState.lastCumulativeWater.toFixed(1)
        : "0.0";
    }
    
    // Override with status if provided and global state doesn't have it
    if (status) {
      if (status.mode && !window.GlobalState) {
        modeLabel = status.mode;
      }
      if (status.pump_status && !window.GlobalState?.getState()?.pumpState) {
        pumpLabel = status.pump_status;
      }
      if (typeof status.water_used === "number" && (!window.GlobalState || !window.GlobalState.getState().lastCumulativeWater)) {
        waterValue = status.water_used.toFixed(1);
      }
    }
    ...
    // Update notification badge from global state
    if (window.GlobalState) {
      const globalState = window.GlobalState.getState();
      const badgeEl = document.querySelector('[data-notif-badge]');
      if (badgeEl && typeof globalState.unreadNotifications === 'number') {
        if (globalState.unreadNotifications > 0) {
          badgeEl.textContent = String(globalState.unreadNotifications);
